...
layui.define(function (exports) {
    ...
    layui.use(["admin", "carousel"], function () {
        var $ = layui.$, carousel = (layui.admin, layui.carousel), element = layui.element, device = layui.device();
    ...
    layui.use(["admin", "carousel", "echarts"], function () {
        var $ = layui.$, admin = layui.admin, carousel = layui.carousel, echarts = layui.echarts, charts = [],
            option = [{
        ...
        var $dataview_div = $("#LAY-index-dataview").children("div");
        // 初始化指定索引图表函数
        var init = function (index) {
            charts[index] = echarts.init($dataview_div[index], layui.echartsTheme);
            charts[index].setOption(option[index]);
        ...
        if ($dataview_div[0]) {
            ...
            var index = 0;
            // 监听轮播切换事件
            carousel.on("change(LAY-index-dataview)", function (event) {
                init(index = event.index);
            });
            // 监听侧边菜单收缩事件
            ...
            // 监听路径hash改变事件，只在单页面有用，在iframe框架中没用
            layui.admin.on("hash(tab)", function () {
                layui.router().path.join("") || init(index);
            });
        }
    });
