...
layui.define(function (exports) {
    ...
    layui.use(["carousel", "echarts"], function () {
        ...
        //没找到DOM，终止执行
        elemDataView[0] && renderDataView(0)
    });

    //访问量
    layui.use(["carousel", "echarts"], function () {
        var $ = layui.$, carousel = layui.carousel, echarts = layui.echarts, echartsApp = [], options = [{
            tooltip: {trigger: "axis"},
            calculable: true,
            ...
            elemDataView = $("#LAY-index-pagetwo").children("div"), renderDataView = function (index) {
            echartsApp[index] = echarts.init(elemDataView[index], layui.echartsTheme);
            echartsApp[index].setOption(options[index]);
            window.onresize = echartsApp[index].resize;
        };
    ...
    layui.use(["carousel", "echarts"], function () {
        ...
        }], elemDataView = $("#LAY-index-pagethree").children("div"), renderDataView = function (index) {
            echartsApp[index] = echarts.init(elemDataView[index], layui.echartsTheme);
            echartsApp[index].setOption(options[index]);
            window.onresize = echartsApp[index].resize;
        };
        ...
        table.render({
            ...
            cols: [[
                ...
                {field: "complete", title: "完成情况", templet: function (e) {
                    ...
                    if("已完成" == e.complete) {
                        return '<del style="color: #5FB878;">' + e.complete + "</del>";
                    }else if("进行中" == e.complete) {
                        return '<span style="color: #FFB800;">' + e.complete + "</span>";
                    }else {
                        return '<span style="color: #FF5722;">' + e.complete + "</span>";
                    }
                }
            }]],
            skin: "line"
