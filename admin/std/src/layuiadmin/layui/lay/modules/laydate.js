...
;!function () {
    ...
    var isLayui = window.layui && layui.define, ready = {
        ...
        , LAY = function (selector) {
            var index = 0
                ...
                this.selector = selector // 保存选择器字符串
                    , document.querySelectorAll(selector || null)
            );
            for (; index < nativeDOM.length; index++) {
                this.push(nativeDOM[index]); // 保存绑定元素
            }
        };
    ...
    */
    ...
    Class.prototype.init = function () {
        ...
        options.elem = lay(options.elem); // 绑定元素
        ...
        if (options.format === format.date) {
            options.format = format[options.type]; // 控件选择类型
        }
    ...
    //生成年月时分秒列表
    Class.prototype.list = function (type, index) { // 控件选择类型
        var that = this
            , options = that.config
