...
layui.define(["layer", "laytpl", "upload"], function (exports) {
    ...
    var v = "3.9.1",
        ...
        LAYIM = function () {
            this.v = v;
            $("body").on("click", "*[layim-event]", function (event) {
                var $this = $(this), methid = $this.attr("layim-event");
                events[methid] ? events[methid].call(this, $this, event) : "";
            })
        };
    ...
    LAYIM.prototype.config = function (options) {
        ...
        options = options || {}; // 初始化选项
        options.skin = options.skin || [];
        
        layui.each(options.skin, function (index, item) {
            skin.unshift(item); // 在数组的头部插入用户自定义的背景图片
        });
        
        options.skin = skin; // 最终的背景图片数组
        options = $.extend({
            isfriend: true,
            isgroup: true,
            voice: "default.mp3" // 消息提示音
        }, options);
    ...
    LAYIM.prototype.setFriendStatus = function (id, type) {
        ...
        list["online" === type ? "removeClass" : "addClass"]("layim-list-gray");
    };
    ...
    var listTpl = function (options) {
            ...
            return [
                ...
                '<li layim-event="chat" data-type="' + options.type + '" data-index="{{ ' + (options.index || "i") + ' }}" class="layim-' + ("history" === options.type ? "{{i}}" : options.type + "{{data.id}}") + ' {{ data.status === "offline" ? "layim-list-gray" : "" }}"><img src="{{ data.avatar }}"><span>{{ data.username||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.sign||"" }}</p><span class="layim-msg-status">new</span></li>',
                "{{# }); if(length === 0){ }}",
    ...
    layui.data.date = function (timestamp) {
        ...
        return date.getFullYear() + "-" + digit(date.getMonth() + 1) + "-" + digit(date.getDate()) + " " + digit(date.getHours()) + ":" + digit(date.getMinutes()) + ":" + digit(date.getSeconds())
    };

    //转换内容，里面可能存在富文本格式内容，需要转换
    layui.data.content = function (content) {
        ...
        content = (content || "").replace(/&(?!#?[a-zA-Z0-9]+;)/g, "&amp;")
            ...
            var href = (str.match(/file\(([\s\S]+?)\)\[/) || [])[1],
                text = (str.match(/\)\[([\s\S]*?)\]/) || [])[1];
            return href ? '<a class="layui-layim-file" href="' + href + '" download target="_blank"><i class="layui-icon">&#xe61e;</i><cite>' + (text || href) + "</cite></a>" : str;
        }).replace(/audio\[([^\s]+?)\]/g, function (audio_src) { //转义音频
            return '<div class="layui-unselect layui-layim-audio" layim-event="playAudio" data-src="' + audio_src.replace(/(^audio\[)|(\]$)/g, "") + '"><i class="layui-icon">&#xe652;</i><p>音频消息</p></div>'
        }).replace(/video\[([^\s]+?)\]/g, function (video_src) { //转义视频
            return '<div class="layui-unselect layui-layim-video" layim-event="playVideo" data-src="' + video_src.replace(/(^video\[)|(\]$)/g, "") + '"><i class="layui-icon">&#xe652;</i></div>'
        }).replace(/a\([\s\S]+?\)\[[\s\S]*?\]/g, function (str) { //转义链接
            var href = (str.match(/a\(([\s\S]+?)\)\[/) || [])[1],
                text = (str.match(/\)\[([\s\S]*?)\]/) || [])[1];
            return href ? '<a href="' + href + '" target="_blank">' + (text || href) + "</a>" : str;
    ...
    var layimMain, // 记录最大化主面板
        layimClose, // 记录最小化主面板
        layimChat, // 记录最大化聊天面板
        layimMin, // 记录最小化聊天面板
        chatIndex,
        //Ajax
        request = function (options, callback, tips) {
            options = options || {};
            return $.ajax({
            url: options.url,
            ...
            success: function (res) {
                0 == res.code ? callback && callback(res.data || {}) : layer.msg(res.msg || (tips || "Error") + ": LAYIM_NOT_GET_DATA", {time: 5000});
            },
    ...
    var cache = {
        ...
        // init参数保存：我的信息、好友列表、群组列表。
        var init = options.init || {};

        mine = init.mine || {}; // 登录者对象
        local = layui.data("layim")[mine.id] || {}; // 获取登录者本地存储的信息
        obj = {
            base: options,
            local: local,
            mine: mine,
        ...
        create = function (data) {
            ...
            // 获取这些数据后，显示在主面板
            popMainPanel(laytpl(elemTpl).render(obj));

            // 如果主面板关闭状态，即最小化状态，则最小化主面板
            ...
            layui.each(call.ready, function (index, item) {
                // object类型的参数，携带一些基础配置信息、我的用户信息、好友/群列表信息、本地数据库信息等
                item && item(obj);
            });
        };
        ...
        // 采用Ajax配置方式或者直接配置方式
        init.url ? request(init, create, 'INIT') : create(init);
    };

    //显示主面板
    var popMainPanel = function (content) {
        return layer.open({
            type: 1,
            area: ["260px", "520px"],
            ...
            success: function (layero) { // 层弹出后的成功回调方法
                ...
                banRightMenu(); //屏蔽主面板右键菜单
                events.sign(); //初始化可编辑的签名
            },
            cancel: function (index) { //右上角关闭按钮触发的回调
                minMainPanel(); // 关闭主窗口弹出最小化窗口
                // 获取当前用户本地数据
    ...
    var banRightMenu = function () {
        ...
        layimMain.find(".layim-list-history").on("contextmenu", "li", function (event) {
            var $this = $(this),
                html = '<ul data-id="' + $this[0].id + '" data-index="' + $this.data("index") + '"><li layim-event="menuHistory" data-type="one">移除该会话</li><li layim-event="menuHistory" data-type="all">清空全部会话列表</li></ul>';

            if($this.hasClass("layim-null")) return;
    ...
    //主面板最小化
    var minMainPanel = function (content) {
        if(layimClose){
            layer.close(layimClose.attr('times'));
        }
        if(layimMain){
        ...
        return layer.open({
            ...
            success: function (layero, index) { // 层弹出后的成功回调方法
                ...
                layero.on("click", function () { // 通过最小化窗口触发弹出主窗口
                    layer.close(index); // 关闭当前层
                    layimMain.show(); // 显示主窗口
                    // 获取当前用户本地数据
                    var local = layui.data("layim")[cache.mine.id] || {};
                    delete local.close;
                    //向localStorage同步数据
    ...
    //显示聊天面板
    var popChatPanel = function (data) {
        data = data || {};
        ...
        if (chat[0]) {
            var list = layimChat.find(".layim-chat-list"),
                listThat = list.find(".layim-chatlist-" + data.type + data.id),
                hasFull = layimChat.find(".layui-layer-max").hasClass("layui-layer-maxmin"),
                chatBox = chat.children(".layim-chat-box");
        ...
        var index = chatIndex = layer.open({
            ...
            success: function (layero) {
                ...
                "function" == typeof data.success && data.success(layero);
                ...
                layero.on("dblclick", ".layui-layim-photos", function () {
                    var src = this.src;
                    layer.close(popChatPanel.photosIndex);
                    layer.photos({
                        photos: {data: [{
                            alt: "大图模式",
                            src: src
                        ...
                        success: function (layero, index) {
                            popChatPanel.photosIndex = index;
                        }
                    });
                });
            },
    ...
    var I = function (data) {
        $(".layim-" + data.type + data.id).each(function () {
            $(this).hasClass("layim-list-gray") && layui.layim.setFriendStatus(data.id, "offline");
        });
    };
    ...
    var setChatMin = function (newMsg) {
        ...
        setChatMin.index = layer.open({
            ...
            success: function (layero, index) {
                ...
                layero.find(".layui-layer-content img").on("click", function (i) {
                    stope(i);
                });
            }
        });
    ...
    var popAdd = function (data, type) {
        ...
        return popAdd.index = layer.open({
            type: 1,
            area: "430px",
            title: {friend: "添加好友", group: "加入群组"}[data.type] || "",
            shade: false,
            resize: false,
        ...
        if (del) {
            ...
            if(length === 1){
                ...
                if(2 === length) {
                    ...
                    hasFull || layimChat.css("width", "600px");
    ...
    var showOffMessage = function () {
        var thatChat = thisChat(),
            message = cache.message[thatChat.data.type + thatChat.data.id];
    ...
    var thisChat = LAYIM.prototype.thisChat = function () {
        if (layimChat) {
            var index = $(".layim-chat-list ." + THIS).index(),
                cont = layimChat.find(".layim-chat").eq(index),
                to = JSON.parse(decodeURIComponent(cont.find(".layim-chat-tool").data("json")));
            return {
                elem: cont,
                data: to,
    ...
    var sendMessage = function () {
        ...
        if ("" !== data.content.replace(/\s/g, "")) {
            ...
            var param = {
                ...
                username: param.mine.username, // 昵称
                avatar: param.mine.avatar, // 头像
                id: param.to.id, // 用户id
                type: param.to.type, // 聊天类型，一般分friend和group两种，group即群聊
                content: param.mine.content, // 消息内容
            ...
            // 执行发送消息事件回调，监听该事件，通过websocket来发送消息给服务器
    ...
    var voice = function () {
        if (!(device.ie && device.ie < 9)) {
            ...
            audio.play(); // 必须先交互，才能执行，不能打开页面直接执行
        }
    };

    //接受消息，监听websocket事件，通过getMessage把接收消息传入到layim中
    //即可显示消息到聊天面板（如果消息所指定的聊天面板没有打开，则会进入本地的消息队列中，直到指定的聊天面板被打开，方可显示。）
    var messageNew = {}, getMessage = function (data) {
        ...
        if (layimChat) {
            ...
            } else if(data.content.replace(/\s/g, '') !== ''){ // 来自普通用户聊天消息
                ul.append(laytpl(elemChatMain).render(data));
            }
    ...
    var pushChatlog = function (message) {
        // 获取当前用户本地数据
        var local = layui.data("layim")[cache.mine.id] || {};
        local.chatlog = local.chatlog || {}; // 本地聊天记录
    ...
    var addList = function (data) {
        ...
        if (!has) {
            var list = laytpl(listTpl({
                type: data.type,
                item: "d.data",
                index: "friend" === data.type ? "data.groupIndex" : null
            })).render({data: obj});
            ...
            if ("friend" === data.type) {
                ...
                //如果初始没有群组
                listElem.find(".layim-null")[0] && listElem.find(".layim-null").remove();
            }
        }
    };
    ...
    var hotkeySend = function () {
        ...
        textarea.off("keydown").on("keydown", function (event) {
            ...
            if ("Ctrl+Enter" === local.sendHotKey) {
    ...
    var anim = "layui-anim-upbit", events = {
        ...
        //编辑用户签名
        sign: function () {
            var $remark = layimMain.find(".layui-layim-remark");
            // 监听签名的改变
            ...
            $remark.on("keyup", function (event) {
        ...
        tab: function (othis) {
            var index, main = ".layim-tab-content",
                tabs = layimMain.find(".layui-layim-tab>li");

            if("number" == typeof othis) {
                index = othis;
                othis = tabs.eq(index);
            }else {
                index = othis.index();
        ...
        spread: function (othis) {
            var type = othis.attr("lay-type"),
                spread = "true" === type ? "false" : "true",
                // 获取当前用户本地数据
                local = layui.data("layim")[cache.mine.id] || {};

            othis.next()["true" === type ? "removeClass" : "addClass"](SHOW);
            ...
            othis.find(".layui-icon").html("true" === spread ? "&#xe61a;" : "&#xe602;");
        },
        //搜索
        search: function (othis) {
            var search = layimMain.find(".layui-layim-search"),
                main = layimMain.find("#layui-layim-search"),
        ...
        skin: function () {
            layer.open({
                ...
                zIndex: 66666666,
                resize: false,
                content: laytpl(elemSkinTpl).render({skin: cache.base.skin})
            });
        },
        ...
        setSkin: function (othis) {
            ...
            try {
                layimMain.css({"background-image": src ? "url(" + src + ")" : "none"});
            ...
            layui.each(call.setSkin, function (index, item) {
                var relativeSrc = (src || "").replace(layui.cache.dir + "css/modules/layim/skin/", "");
                item && item(relativeSrc, src);
            })
        },
        //弹出聊天面板
        chat: function (othis) {
            ...
            var local = layui.data("layim")[cache.mine.id] || {},
                ...
                list = othis.attr("data-list") || othis.index(),
                data = {};
                ...
                events.groupMembers.index = layer.tips('<ul class="layim-members-list"></ul>', othis, {
                    ...
                    success: function (layero) {
                        var members = cache.base.members || {},
                            ...
                            membersCache = {},
                            hasFull = layimChat.find(".layui-layer-max").hasClass("layui-layer-maxmin"),
                            listNone = "none" === layimChat.find(".layim-chat-list").css("display");
        ...
        face: function (othis, e) {
            ...
            for (var s in faces) {
                content += '<li title="' + s + '"><img src="' + faces[s] + '"></li>';
            }
            ...
            events.face.index = layer.tips(content, othis, {
                ...
                success: function (layero) {
                    layero.find(".layim-face-list>li").on("mousedown", function (i) {
                        stope(i);
                    }).on("click", function () {
        ...
        image: function (othis) {
            var type = othis.data("type") || "images",
                api = {images: "uploadImage", file: "uploadFile"},
                thatChat = thisChat(),
            ...
            layui.upload.render({
                ...
                accept: type,
                    ...
                    if(0 == i.code) {
                        ...
                        if("images" === type) {
                            focusInsert(thatChat.textarea[0], "img[" + (i.data.src || "") + "]");
                        }else if("file" === type){
                            focusInsert(thatChat.textarea[0], "file(" + (i.data.src || "") + ")[" + (i.data.name || "下载文件") + "]");
                        }

                        sendMessage();
                    }else {
                        layer.msg(i.msg || "上传失败");
                    }
                }
            });
        },
        ...
        media: function (othis) {
            var type = othis.data("type"),
                api = {audio: "音频", video: "视频"},
                thatChat = thisChat();

            layer.prompt({
                title: "请输入网络" + api[type] + "地址",
                shade: false,
                offset: [othis.offset().top - $(window).scrollTop() - 158 + "px", othis.offset().left + "px"]
            }, function (value, index) {
                focusInsert(thatChat.textarea[0], type + "[" + value + "]");
                sendMessage();
                layer.close(index);
            });
        },
        ...
        playAudio: function (othis) {
            var audio = othis.data("audio"),
                audioDom = audio || document.createElement("audio"),
                pause = function () {
                    audioDom.pause();
            ...
            }else if(audioDom.play) {

                if(othis.attr("status")) {
                    pause();
                }else {
                    if(!audio) {
                        audioDom.src = othis.data("src");
                    }
                    audioDom.play();
                    othis.attr("status", "pause");
                    othis.data("audio", audioDom);
                    othis.find("i").html("&#xe651;");
        ...
        playVideo: function (othis) {
            var src = othis.data("src"), videoDom = document.createElement("video");
        ...
        menuHistory: function (othis, e) {
            ...
            var local = layui.data("layim")[cache.mine.id] || {},
                parent = othis.parent(),
                type = othis.data("type"),
                hisElem = layimMain.find(".layim-list-history"),
                none = '<li class="layim-null">暂无历史会话</li>';
